...
#define ADMIN_GODMODE 160
#define ADMIN_ADMINLEVEL 71
#define ADMIN_ADMINLEVEL2 72
#define PLAYER_STATS 73
...
enum pInfo
{
	...
	pVirtualWorld,
	pSkin,
	pRace,   // 0 = Human,  1 = Vampire,  2 = Android, 3 = Mage
	pVampireJump,
	Float:pEXPBarValue,
	pEXP,
	pMaxEXP,
...
public LoadUser_data(playerid,name[],value[])
{
	...
	INI_Int("pAP", PlayerInfo[playerid][pAP]);
	INI_Int("pAdminLevel", PlayerInfo[playerid][pAdminLevel]);
	return 1;
}
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    switch( dialogid )
    {
        case DIALOG_REGISTER:
        {
            ...
            if(response)
            {
				...
				INI_WriteInt(File, "pAdmintwo", 0);
				INI_WriteInt(File, "pLevel", 1);
				INI_WriteInt(File, "pAP", 0);
        ...
        case DIALOG_LOGIN:
        {
            ...
            if( response )
            {
                ...
                if(PlayerInfo[playerid][pAdminLevel]>0) {
                    new string[129];
                    format(string, sizeof(string), "[LOGIN] You are logged in as a level %d Admin.", PlayerInfo[playerid][pAdminLevel]);
	...
	if(dialogid == 60 && response == 1){
		...
		case 7: ShowPlayerDialog(playerid, ADMIN_GODMODE, DIALOG_STYLE_INPUT, "GODEMODE", "Does not matter:", "Confirm", "Cancel");
		case 8: ShowPlayerDialog(playerid, ADMIN_ADMINLEVEL, DIALOG_STYLE_INPUT, "ADMIN LEVEL", "Target:", "Confirm", "Cancel");
		}
	}

	if(dialogid == ADMIN_ADMINLEVEL && response == 1)
	{
	    if(IsPlayerAdmin(playerid)){
	        Assign[temp01]=strval(inputtext);
	        ShowPlayerDialog(playerid, ADMIN_ADMINLEVEL2, DIALOG_STYLE_INPUT, "ADMINLEVEL", "Enter Admin LEVEL:", "Confirm", "Cancel");

	    }
	    else
	    {
	        SendClientMessage(playerid, COLOR_CYAN, "ERROR: YOU ARE NOT A HIGH ENOUGH ADMIN!");
		}


	}

	if(dialogid == ADMIN_ADMINLEVEL2 && response == 1)
	{
	    Assign[temp02]=strval(inputtext);
		PlayerInfo[Assign[temp01]][pAdminLevel]=Assign[temp02];
		SendClientMessage(playerid, COLOR_CYAN, "[ADMIN] You have changed the player's admin level.");
		SendClientMessage(Assign[temp01], COLOR_CYAN, "[Character] Your admin level has been changed.");
	}

	if(dialogid == ADMIN_GODMODE && response == 1)
	{
	    if(PlayerInfo[playerid][pAdmintwo]==0)
	    {
	        SendClientMessage(playerid, COLOR_CYAN,"[Admin] You are in god mode.");
	        PlayerInfo[playerid][pAdmintwo]=1;
	    }
	    else
	    {
	        SendClientMessage(playerid, COLOR_CYAN, "[Admin] You are not in god mode.");
	        PlayerInfo[playerid][pAdmintwo]=0;
	    }
	}
	...
	{   if(PlayerInfo[playerid][pRace]==1 && PlayerInfo[playerid][pAP]>0 && PlayerInfo[playerid][pVampireJump]<4){
	        ...
	        ShowPlayerSkills(playerid);
		} else if(PlayerInfo[playerid][pRace]!=1) {

		    SendClientMessage(playerid, COLOR_CYAN, "ERROR: You are not a {FFFFFF}vampire{FFFFFF}.");
		} else if(PlayerInfo[playerid][pAP]<0) {
		    SendClientMessage(playerid, COLOR_CYAN, "ERROR: You do not have enough ABILITY POINTS.");
		} else if(PlayerInfo[playerid][pVampireJump]==3) {
		    SendClientMessage(playerid, COLOR_CYAN, "ERROR: You have maxed this skill.");
		}
	}
	...
	if(dialogid == PLAYER_STATS && response == 1)
	{
...
public OnPlayerDisconnect(playerid, reason)
{
	...
	INI_WriteInt(File, "pAdmintwo", PlayerInfo[playerid][pAdmintwo]);
	INI_WriteInt(File, "pLevel", PlayerInfo[playerid][pLevel]);
	INI_WriteInt(File, "pAP", PlayerInfo[playerid][pLevel]);
	INI_WriteInt(File, "pAdminLevel", PlayerInfo[playerid][pAdminLevel]);
...
public OnPlayerDeath(playerid, killerid, reason)
{
    ...
    new var;
    for(var=0;var<MAX_PLAYERS+1;var++){   // Sends a message to admins that a person has been killed.
        if(PlayerInfo[var][pAdminLevel]>0){
            new string[129], victim[129], murderer[129];
            GetPlayerName(playerid, victim);
            GetPlayerName(killerid, murderer);
            format(string, sizeof(string), "[ADMIN KILL LOG] [ID:%d]%s killed by [ID:%d]%s.", playerid, victim, killerid, murderer);
            SendClientMessage(var, COLOR_CYAN, string);
        }
    
    SendClientMessage(playerid, COLOR_CYAN, "[Character] You have been killed.");
    SendClientMessage(killerid, COLOR_CYAN, "[Character] You have killed a person.");
...
public JumpDisable(playerid){
	Assign[temp01]=PlayerInfo[playerid][pVampireJump];
	PlayerInfo[playerid][pVampireJump]=0;
...
public JumpEnable(playerid) {
	PlayerInfo[playerid][pVampireJump]=Assign[temp01];
...
CMD:admin(playerid, params[])
{
	...
	{
	    SendClientMessage(playerid, COLOR_CYAN, "ERROR: You are not an admin.");
	}
	return 1;
}
...
CMD:levelup(playerid, params[])
{
	if(PlayerInfo[playerid][pEXP]==PlayerInfo[playerid][pMaxEXP])
	{
	    ...
	    PlayerInfo[playerid][pMaxEXP]=(PlayerInfo[playerid][pMaxEXP]*2)-(((PlayerInfo[playerid][pLevel]-1)*2));
	    newMax=PlayerInfo[playerid][pMaxEXP];
	    PlayerInfo[playerid][pMaxEXPBarValue]=newMax;
	    ChangeEXP(playerid);
...
CMD:kick(playerid, params[])
{
    if(PlayerInfo[playerid][pAdminLevel]>0){
        ...
        SendClientMessage(playerid, COLOR_CYAN, "[Error] You are not an administrator.");
    }
    return 1;
}
...
CMD:race(playerid, params[])
{
	if(PlayerInfo[playerid][pRace]==0){
	    SendClientMessage(playerid, COLOR_CYAN, "[Character] You are a human being.");
	} else if(PlayerInfo[playerid][pRace]==1){
	    SendClientMessage(playerid, COLOR_CYAN, "[Character] You are a vampire.");
	} else if(PlayerInfo[playerid][pRace]==2){
	    SendClientMessage(playerid, COLOR_CYAN, "[Character] You are an android.");
	} else if(PlayerInfo[playerid][pRace]==3){
	    SendClientMessage(playerid, COLOR_CYAN, "[Character] You are a mage.");
	} else {
	    SendClientMessage(playerid, COLOR_CYAN, "[Character] You are not of a known species.");
	}
	return 1;
}
...
showstats(playerid){
	new string[200];
	new gname[129], grace[129], gmoney, gdeath, gkill;

	GetPlayerName(playerid, gname, sizeof(gname));

	if(PlayerInfo[playerid][pRace]==0){
	    format(grace, sizeof(grace), "%s", "Human");
	} else if(PlayerInfo[playerid][pRace]==1){
	    format(grace, sizeof(grace), "%s", "Vampire");
	} else if(PlayerInfo[playerid][pRace]==2){
	    format(grace, sizeof(grace), "%s", "Android");
	} else if(PlayerInfo[playerid][pRace]==3){
	    format(grace, sizeof(grace), "%s", "Mage");
	} else {
	    format(grace, sizeof(grace), "%s", "Unknown");
	}

	gmoney=GetPlayerMoney(playerid);
	gdeath=PlayerInfo[playerid][pDeaths];
	gkill=PlayerInfo[playerid][pKills];
	
	format(string, sizeof(string), "Name: %s\nRace: %s\nMoney: %d\nDeaths: %d\nKills: %d", gname, grace, gmoney, gdeath, gkill);
	
	ShowPlayerDialog(playerid, PLAYER_STATS, DIALOG_STYLE_LIST, "Character Stats", string, "Confirm", "Back");
...
ChangeEXP(playerid)
{
	...
	SetPlayerProgressBarMaxValue(playerid, PlayerBar:pEXPBarID[playerid], PlayerInfo[playerid][pMaxEXPBarValue]);
...
DisplayLevel(playerid)
{
	...
	levelstat[playerid] = CreatePlayerTextDraw(playerid, 133.600021, 409.919982, string);
...
ChangeLevel(playerid)
{
	new string[129];
	format(string, sizeof(string), "Level: %d", PlayerInfo[playerid][pLevel]);
	PlayerTextDrawSetString(playerid, PlayerText:levelstat[playerid], string);
}
...
{
   	new string[250];
	format(string, sizeof(string), ""COL_CYAN"TOTAL ABILITY POINTS: %d\n"COL_INDIGO"\t\t***General Skills***\n"COL_GREEN"\t\t***Human Skills***\n"COL_MAROON"\t\t***Vampire Skills***\nVampireJump(%d/3)", PlayerInfo[playerid][pAP], PlayerInfo[playerid][pVampireJump]);
	ShowPlayerDialog(playerid, SKILL_DIALOG, DIALOG_STYLE_LIST, "Skills", string, "Select", "Cancel");
}

public OnPlayerTakeDamage(playerid, issuerid, Float: amount, weaponid, bodypart)
{
	if(IsPlayerAdmin(playerid) && PlayerInfo[playerid][pAdmintwo]==1)
	{
